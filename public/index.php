...
$resp = curl_exec($curl);
$wirelessData = json_decode($resp,true);
curl_close($curl);
...
if ($groupId != "")
{
    ...
    curl_close($curl);
    if (sizeof($allEndPoints["endpointProfilesBody"]) == 0) $app->render('configure.twig',array("userName" => $userName));
    else $app->render('configure.twig',array("endPointsData" => $endPointsData,"userName" => $userName, "wirelessData" => $wirelessData));
}
else
{
    $app->render('configure.twig',array("userName" => $userName));
}
    ...
    $app->post('/sendCommand/', function ($key) use ($app) {
        $app->render('users/zerg.twig'); 
    });  
